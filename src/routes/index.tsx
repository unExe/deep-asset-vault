        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-1">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 text-xs text-vault-fg-muted hover:text-vault-fg"
            >
              YouTube
            </a>
            <a
            ...
            <Link
              to="/vault"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-vault-overlay-strong hover:bg-vault-card-hover text-xs text-vault-fg border border-vault-hairline"
            >
              <Vault size={13} /> Vault
            </Link>
            <ThemeToggle />
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <section className="pt-12 sm:pt-20 pb-16 sm:pb-24 grid lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          <div className="lg:col-span-7">
            ...
            <h1 className="mt-5 text-6xl sm:text-8xl font-semibold tracking-tight leading-[0.9]">
              .unExe
            </h1>
            <p className="mt-6 max-w-xl text-base sm:text-lg text-vault-fg-muted leading-relaxed">
              I make short-form content. This site is the home of every asset I
              use — overlays, sounds, presets, and project files. Browse the
              vault, preview anything, download what you need.
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-2.5">
              <Link
                to="/vault"
                className="inline-flex items-center gap-2 px-5 py-3 rounded-md bg-vault-fg text-vault-bg text-sm font-medium hover:opacity-90 transition-opacity"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 px-5 py-3 rounded-md bg-vault-overlay-strong hover:bg-vault-card-hover text-sm font-medium border border-vault-hairline"
              >
                <YoutubeLogo size={16} weight="fill" /> Watch
                <ArrowUpRight size={13} />
              </a>
              <a
                href={TG_URL}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 px-5 py-3 rounded-md bg-vault-overlay hover:bg-vault-overlay-strong text-sm font-medium border border-vault-hairline"
              >
          ...
          <div className="lg:col-span-5">
            <div className="relative aspect-[4/5] rounded-2xl overflow-hidden border border-vault-hairline bg-vault-overlay">
              <img
                src={heroPattern}
                alt="Abstract flowing line pattern representing motion and editing"
                className="w-full h-full object-cover"
                loading="eager"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-vault-bg/80 via-transparent to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <div className="flex items-center gap-2 text-[11px] uppercase tracking-wider text-vault-fg-muted">
                  <Sparkle size={12} weight="fill" /> latest drop
                </div>
                <p className="mt-1.5 text-vault-fg font-medium text-sm">
                  Overlays · transitions · SFX
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="py-12 sm:py-16 border-t border-vault-hairline">
          <div className="flex items-end justify-between mb-8 gap-4 flex-wrap">
            <div>
              <span className="text-[11px] uppercase tracking-wider text-vault-fg-muted">
                what's inside
              </span>
              <h2 className="mt-2 text-3xl sm:text-4xl font-semibold tracking-tight">
                Every asset, organized.
              </h2>
            </div>
            <Link
              to="/vault"
              className="text-sm text-vault-fg-muted hover:text-vault-fg inline-flex items-center gap-1"
            >
        ...
        <section className="py-12 sm:py-16 border-t border-vault-hairline grid lg:grid-cols-2 gap-10 items-start">
          <div>
            ...
            <p className="mt-5 text-vault-fg-muted leading-relaxed">
              I edit short-form content — fast cuts, layered overlays, and
              sound design built for the scroll. This site exists for one
              reason: to give you everything I use, in one place, without the
              Discord-server scavenger hunt.
            </p>
            <p className="mt-4 text-vault-fg-muted leading-relaxed">
              No paywalls. No watermarks. Just the vault.
            </p>
          </div>
        ...
        <section className="py-12 sm:py-16 border-t border-vault-hairline">
          ...
          <div className="mt-8 grid sm:grid-cols-2 gap-3">
            <ChannelCard
              ...
              title="YouTube"
              handle="@unexecutable"
              desc="The shorts. New uploads regularly."
            />
            <ChannelCard
              href={TG_URL}
              ...
              desc="Drops, sneak peeks, and direct conversation."
            />
          </div>
        </section>
        ...
        <section className="py-16 sm:py-24 text-center border-t border-vault-hairline">
          ...
          <p className="mt-4 text-vault-fg-muted max-w-md mx-auto">
            Open the vault and grab whatever you need.
          </p>
          <Link
            to="/vault"
            className="mt-7 inline-flex items-center gap-2 px-6 py-3.5 rounded-md bg-vault-fg text-vault-bg text-sm font-medium hover:opacity-90"
          >
            <Vault size={16} /> Open vault.unExe
            <ArrowUpRight size={13} />
          </Link>
        </section>
        ...
        <footer className="border-t border-vault-hairline py-8 text-xs text-vault-fg-muted flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            <a href={YT_URL} target="_blank" rel="noreferrer" className="hover:text-vault-fg">
              YouTube
            </a>
            <a href={TG_URL} target="_blank" rel="noreferrer" className="hover:text-vault-fg">
              Telegram
            </a>
            <Link to="/vault" className="hover:text-vault-fg">
              Vault
            </Link>
          </div>
        </footer>
...
function ChannelCard({
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="group rounded-lg border border-vault-hairline bg-vault-overlay p-5 sm:p-6 hover:bg-vault-overlay-strong transition-colors flex items-start gap-4"
    >
      <div className="w-12 h-12 rounded-md bg-vault-overlay-strong flex items-center justify-center text-vault-fg shrink-0">
        {icon}
      </div>
